...
| AI worker callback | 별도 인증 header 확인되지 않음 | `/api/ai/**` | Security상 공개 |
...
`DeviceResDto`와 `PumpChannelResDto`에는 장치 인증 키를 응답하는 필드가 구현되어 있습니다. 이 문서는 값을 기록하지 않으며, API 설계상 해당 필드를 노출하지 않도록 변경하는 것이 필요합니다. 또한 목록 API는 코드 주석상 전체 데이터를 반환하므로 일반 JWT 사용자에게 장치 구성이 노출될 수 있습니다.
...
| 중요도 | 항목 | 코드 근거 | 조치 필요 |
| --- | --- | --- | --- |
| 높음 | 장치 구성 응답이 인증 키 필드를 포함 | `IotSetupDto.DeviceResDto`, `PumpChannelResDto` | API 응답에서 credential 제거 |
| 높음 | AI callback 공개 | `SecurityConfig`, `AiPlantImageController` | worker 서명/서비스 인증 도입 |
...
GreenLink의 중심 API는 Spring Backend가 제공하며, Flutter는 JWT API를, ESP와 Pi는 장치 키 API를, Ubuntu AI Worker는 공개 callback API를 사용합니다. Pi는 별도의 MJPEG stream과 AI 작업 trigger 역할도 맡습니다.

현재 코드 기준 명세에서 즉시 조정이 필요한 계약은 sensor refresh 미구현, 자동화 필드 불일치, 급수 시간 설명 불일치입니다. 운영 보안 측면에서는 장치 키 응답 노출과 AI callback 인증 부재가 핵심 확인 사항입니다.
