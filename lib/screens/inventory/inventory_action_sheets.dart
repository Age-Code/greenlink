...
// PotEquipBottomSheet
// TEST 7: 화분 장착
//   - GET /api/user-plants (전체 조회)
//   - GROWING / HARVESTABLE → 선택 가능
//   - HARVESTED → 흐리게 + 선택 불가
//   - POST /api/user-items/{userItemId}/equip-pot
// ============================================================
...
class _PotEquipBottomSheetState extends State<PotEquipBottomSheet> {
  ...
  void _loadPlants() async {
    // TEST 7: 전체 조회 후 프론트에서 HARVESTED 비활성 처리
          ...
          children: [
            ...
            if (_plants != null && _plants!.isNotEmpty) ...[
              ...
              GreenlinkButton(
                ...
                onPressed: _selectedPlantId != null ? _equip : null,
              )
            ]
          ],
        ),
...
// ============================================================
// NutrientUseBottomSheet
// TEST 8: 영양제 사용
//   - GET /api/user-plants (전체 조회)
//   - GROWING / HARVESTABLE → 선택 가능
//   - HARVESTED → 흐리게 + 선택 불가
//   - POST /api/user-items/{userItemId}/use-nutrient
// ============================================================
...
class _NutrientUseBottomSheetState extends State<NutrientUseBottomSheet> {
  ...
  void _loadPlants() async {
    // TEST 8: 전체 조회 후 프론트에서 HARVESTED 비활성 처리
    final res = await _plantService.getUserPlants();
    if (!mounted) return;
    setState(() {
      _plants = res.data ?? [];
          ...
          children: [
            ...
            if (_plants != null && _plants!.isNotEmpty) ...[
              ...
              GreenlinkButton(
                text: "영양제 주기",
                isLoading: _isUsing,
                onPressed: _selectedPlantId != null ? _useNutrient : null,
              )
            ]
          ],
        ),
